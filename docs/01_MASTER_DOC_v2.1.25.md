...
## [DESIGN:COMPONENTS]
shadcn-vue component system - Copy-paste components with Tailwind + Radix Vue
[L1600]

### shadcn-vue Core Components (v2025-11-24)

**Installation:**
```bash
npm install clsx tailwind-merge class-variance-authority radix-vue
```

**Core Components:**
1. **Button.vue** - Primary CTA with variants (default, secondary, outline, ghost, link, destructive)
2. **Card.vue** - Card container (Card, CardHeader, CardContent, CardFooter)
3. **Input.vue** - Form input with v-model and accessibility
4. **Badge.vue** - Tag/label display with variants
5. **Dialog.vue** - Modal dialog using Radix Vue

**App-Specific Components:**
- **NoteCard.vue** - Note display card (uses Card + Badge)
- **TagBadge.vue** - Tag display with remove button
- **TagInput.vue** - Existing autocomplete component (can be refactored)
...
│   ├── NoteCard.vue (app-specific)
│   ├── TagBadge.vue (app-specific)
...
**Benefits:**
- 0KB bundle overhead (copy-paste, not npm package)
- 100% control over component code
- Built on Tailwind CSS (matches design system)
...
#### TagInput.vue
- Autocomplete tag selector with debounce
- Uses @vueuse/core for debouncing
- Can be refactored to use new Input + Badge components
